package flashcards.genalion.aquizo.viewmodel
...
import flashcards.genalion.aquizo.room.FlashcardsDao
import flashcards.genalion.aquizo.room.FlashcardsSet
import flashcards.genalion.aquizo.room.Word
import flashcards.genalion.aquizo.workers.DeleteFlashcardWorker
import flashcards.genalion.aquizo.workers.DeleteSetWorker
import flashcards.genalion.aquizo.workers.InsertFlashcardWorker
import flashcards.genalion.aquizo.workers.UpdateFlashcardWorker
import flashcards.genalion.aquizo.workers.UpdateSetWorker
...
class FlashcardsViewModel(private val flashcardsDao: FlashcardsDao, private val application: Application): ViewModel() {
    ...
    private val _sortSetOption = MutableLiveData<Int>()
    val sortSetOption: LiveData<Int> get() = _sortSetOption
    fun setSortSetOption(option: String) {
        try {
            _sortSetOption.value = option.toInt()
        } catch(e: NumberFormatException) {
            Log.e(TAG, "Incorrect sort option")
            _sortSetOption.value = 1
        }
    }
    ...
    fun sortFlashcardsSet() {
        if(flashcardsSets.value != null) {
            when(sortSetOption.value) {
                0 -> _flashcardsSetsSorted.value = flashcardsSets.value
                1 -> _flashcardsSetsSorted.value = flashcardsSets.value!!.reversed()
                2 -> _flashcardsSetsSorted.value = flashcardsSets.value!!.sortedBy { it.name }
                3 -> _flashcardsSetsSorted.value = flashcardsSets.value!!.sortedBy { it.name }.reversed()
                else -> {  }
            }
        }
    }
    ...
    // flashcards to learn from
    private val _learnOption = MutableLiveData<Int>()
    fun setLearnOption(option: Int) { _learnOption.value = option }
    ...
    fun setLearnFlashcards(flashcards: List<Word>) {
        if(_firstLearnFlashcardsInitialization.value!!
            ...
            when(_learnOption.value) {
                0 -> {  }
                1 -> { newLearnFlashcards = newLearnFlashcards.reversed() }
                2 -> { newLearnFlashcards = newLearnFlashcards.shuffled() }
                3 -> { newLearnFlashcards = newLearnFlashcards.sortedBy { it.term } }
                4 -> { newLearnFlashcards = newLearnFlashcards.sortedBy { it.term }.reversed() }
            }
    ...
    fun getNextToLearn() {
        ...
        if(currentLearnIndex.value == learnFlashcards.value!!.size) {
            ...
            _currentLearnIndex.value = -1
            if(_learnOption.value == 2) {
                if(_nextLearnFlashcards.value!!.size > 1) {

                    val last = _nextLearnFlashcards.value!!.last()
                    Log.d(TAG, "Last: $last")
                    var shuffled = _nextLearnFlashcards.value!!.shuffled()
                    Log.d(TAG, "Shuffled: $shuffled")
                    while(last == shuffled.first()) {
                        shuffled = _nextLearnFlashcards.value!!.shuffled()
                        Log.d(TAG, "Shuffled: $shuffled")
                    }
                    _learnFlashcards.value = shuffled
                }

                else _learnFlashcards.value = _nextLearnFlashcards.value!!
    ...
    fun changeLearnedValueInRoom(learned: Boolean) {
        ...
        if (learned && !flashcardToUpdate.learned && currentSet.value!!.wordsCount > currentSet.value!!.learnedCount) updatedSet.learnedCount += 1
        if (!learned && flashcardToUpdate.learned && currentSet.value!!.learnedCount > 0) updatedSet.learnedCount -= 1
        ...
        // if we have all flashcard option, add the current flashcard to the next learn flashcards list
            ...
            _nextLearnFlashcards.value!!.add(flashcardToUpdate)
        }
    ...
    private val _sortFlashcardsOption = MutableLiveData<Int>()
    val sortFlashcardsOption: LiveData<Int> get() = _sortFlashcardsOption
    fun setSortFlashcardsOption(option: String) {
        try {
            _sortFlashcardsOption.value = option.toInt()
        } catch (e: NumberFormatException) {
            Log.e(TAG, "Incorrect sort option")
            _sortFlashcardsOption.value = 1
        }
    }
    ...
    fun sortFlashcards() {
        if(flashcardsToDisplay.value != null) {
            when(sortFlashcardsOption.value) {
                0 -> { _flashcardsSorted.value = flashcardsToDisplay.value }
                1 -> { _flashcardsSorted.value = flashcardsToDisplay.value!!.reversed() }
                2 -> { _flashcardsSorted.value = flashcardsToDisplay.value!!.sortedBy { it.term } }
                3 -> { _flashcardsSorted.value = flashcardsToDisplay.value!!.sortedBy { it.term }.reversed() }
            }
        }
    }
